...
//  bootstrap-1 -m estate/1-estate-workspace-with-persona.md -f 2
//  2. Diagram
//  cargo install --bin bootstrap-1 --path .
...
lazy_static! {
		static ref PATHS: HashMap<&'static str, &'static str> = {
				...
				m.insert("diagrams", PIPELINE_DIAGRAM);
				m.insert("workspace-1", PIPELINE_ESTATE_WORKSPACE);
				// Add more insertions here if needed
				m // Return the map
		};
}
lazy_static! {
		static ref FEATURE_FLAG: HashMap<&'static str, bool> = {
				let mut m = HashMap::new();
				m.insert("log-verbose", false);
				// Add more insertions here if needed
				m // Return the map
		};
}
...
///###### The format must always align the text
///#####  The import part is that
///####   Between the "required" and "optional" content left of the column aligned by this formatter.
///###    The hashmarks are not just randomly inserted. They're automatcally shifted by the formatted if it sees
///##     a prefix # on a line inside of a "///-" (and expanded form) block
///#      In this way, I can add a hash tag, and when its done, I know this will safely parse to H1 in .md, this line here.
///#      H1 (Goal) This is the goal.
///##     H2 (Goal) I added the hash tags on these lines.
///###    H3 (Goal) I want them to be shift left, but also respect my alignment
///####   H4 (Goal)
///#####  H5 (Goal)
///###### H6 (Goal)
///------------------------------------------------------------------------------------
///--------------------------------------------------------------------------------
///       I get this free. From IDE and LSP. Alignement needs help though.
///       If there are chars between "^///" and the 10 column of a row, slide them to the 10 column. Otherwise, if there chars afterwhere
///       from 10 onwards, do not ouch them.
///######
///       I've met min requirments to 'not be touched'. So I never move
///         - So have I
///             - Me too. I have no chars between ^/// and index 10 of this column and have some afterwards
///#      Compiler Entrypoint
///##     desc: How to interpret this section...?
///       - Right now, it's as a "plan/idea/list" paths/dirs representing an estate workspace for a single user persona.
///#      Developer, PM, Baker could all benefit from estate concept.
///##     But the "stage" concept applied to .md means kind needs to evolve in what well?
///###    The kind should tell us how each sections nested bulletpoint are treated.
///       - Developer persona
///             - Stage 1:  treats a bullet list as a list of files to make
///             - Stage 2:  treats a bullet list as a list of files to extract comment sections from. Imagine .rs files generating .md from their inline comment blocks
///       - Writer persona:
///         - Stage 1:  treats a bullet list as a list of diagrams to make
///         - Stage 2:  treats sections as styles to normalize. Imagine looking through all ```mermaid blocks and collecting colors to normalize
///       - YouTube persona:
///         - Stage 2: Generate diagrams for animation in .py
///         - Stage 1: Animate diagrams using https://github.com/manimCommunity/manim
///
///------------------------------------------------------------------------------------
