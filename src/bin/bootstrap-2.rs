...
pub fn parse_estate_markdown(content: &str) -> Vec<EstateSection> {
  ...
  for line in content.lines() {
    ...
    if in_manifest_list && (trimmed.starts_with('-') || trimmed.starts_with('*')) {
      let raw_item = trimmed.trim_start_matches('-').trim_start_matches('*').trim();
...
pub fn materialize_payload(estate_file_path: &Path) -> std::io::Result<()> {
  println!("📦 Reading payload manifest from: {}", estate_file_path.display());
  if !estate_file_path.exists() {
    return Err(
      std::io::Error::new(
        std::io::ErrorKind::NotFound,
        format!("Payload manifest not found at {}", estate_file_path.display())
      )
    );
  }
