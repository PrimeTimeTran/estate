...
impl Sdlc {
	...
	async fn generate_tests(&self, intent: &str, spec: &str, plan: &str) -> Result<String> {
		let prompt = structure_prompt_for_generate_tests(intent, spec, plan);
		self.generator.generate(&prompt).await
	}
	...
	pub fn create_dir(&self, title: &str) -> Result<PathBuf> {
		let root = workspace_cargo_path().join("log").join("session");
	...
	fn initialize_templates(&self, dir: &Path) -> Result<()> {
		let template_dir = workspace_cargo_path().join("ai").join("template");
	...
	fn record_session(&self) -> Result<()> {
		let session = self
			...
			.as_ref()
			.ok_or_else(|| anyhow::anyhow!("no active SDLC session"))?;
		let log_dir = workspace_cargo_path().join("log");
	...
	pub async fn run(&mut self) -> Result<()> {
		...
		loop {
			let stage = match self.stage() {
				...
				None => {
					println!("SDLC EXIT: no current stage");
					println!("total elapsed: {}", format_elapsed(run_started.elapsed()));
					return Ok(());
				}
			};

			if last_stage == Some(stage.clone()) {
				attempt += 1;
			} else {
				attempt = 1;
			...
			println!();
			println!("══════════════════════════════════════════════");
			println!("SDLC {stage:?} | attempt #{attempt}/{MAX_STAGE_ATTEMPTS}");
			println!("elapsed: {}", format_elapsed(run_started.elapsed()));
			println!("══════════════════════════════════════════════");
			...
			let execution_result = match stage.clone() {
				...
				Stage::Complete => {
					println!("→ Entering Complete");
					self.clear_current()?;

					println!();
					println!("┌──────────────────────────────────────────────");
					println!("│ SDLC RUN COMPLETE");
					println!("│ total elapsed: {}", format_elapsed(run_started.elapsed()));
					println!("└──────────────────────────────────────────────");
			...
			println!("✓ Stage execution complete");

			let decision = match self.evaluate_stage(stage.clone()).await {
				Ok(decision) => decision,

				Err(error) => {
					println!("✗ JEV EVALUATION FAILED");
					println!("  stage: {stage:?}");
					println!("  error: {error}");
					...
					println!("↻ retrying stage because evaluation failed");
					continue;
				}
			};
			...
			let action = if decision.passed {
				StageAction::Continue
			} else if attempt < MAX_STAGE_ATTEMPTS {
				StageAction::Retry
			} else {
				StageAction::Abort
			};
			...
			match action {
				StageAction::Continue => {
					...
					println!("→ JEV says stage passed");
					println!("→ transition: {stage:?} → {next:?}");

					self.transition(next)?;
				}

				StageAction::Retry => {
					println!("↻ RETRY: {stage:?} | next attempt #{}", attempt + 1);
				...
				StageAction::Abort => {
					println!("✗ ABORT: {stage:?}");
					println!("  exhausted {} attempts", MAX_STAGE_ATTEMPTS);

					return Err(anyhow!(
						"SDLC aborted at {stage:?} after {attempt} attempts"
					));
				}
			}
		}
...
impl Sdlc {
	pub fn retry(&mut self, stage: Stage) -> Result<()> {
		self.stage_attempt += 1;

		println!("↻ RETRY {stage:?} | attempt #{}", self.stage_attempt);
...
fn structure_prompt_for_generate_tests(intent: &str, spec: &str, plan: &str) -> String {
	format!(
		r#"
