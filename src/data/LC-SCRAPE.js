...
function grabTestCases2() {
  return [...document.querySelectorAll("div.example-block")].map((pre, index) => {
...
function copyProblemWithDesc() {
  ...
  const paragraphs = description ? [...description.querySelectorAll("p")].slice(0, 2) : [];

  const descriptionHtml = paragraphs.map((p) => p.outerHTML).join("\n");
  const descriptionText = paragraphs.map((p) => normalize(p.textContent)).join("\n\n");
